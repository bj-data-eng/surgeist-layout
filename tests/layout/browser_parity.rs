...
fn browser_parity_html_corpus_inventory_is_documented() {
    ...
    assert_eq!(
        taffy_plus_local_count, 1121,
        "expected the Taffy baseline plus six Surgeist constrained additions and twelve BR coverage fixtures"
    );
...
fn browser_parity_generation_report_counts_full_scope() {
    ...
    assert_eq!(report_json["summary"]["generated"], 5032);
