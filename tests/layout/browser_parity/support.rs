...
impl Golden {
    ...
    pub fn parse(raw: &str) -> Result<Self, Error> {
        ...
        let root = parse_node(one_element_child(input)?)?;
        validate_fri06_c08r_explicit_input(&root, true)?;
        let expectations = parse_expectation(one_element_child(expectations)?)?;
