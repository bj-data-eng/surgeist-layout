...
mod tests {
    ...
    fn test_manifest_with_declared_fixture() -> String {
        let mut manifest = toml::from_str::<toml::Table>(&test_schema_two_manifest(""))
            .expect("test manifest table");
        let mut declared_fixture = toml::from_str::<toml::Table>(
            r#"[[cases]]
id = "block/example"
...
"#,
        ...
        .expect("declared fixture table");
        manifest.insert(
            "cases".to_string(),
            declared_fixture
                .remove("cases")
                .expect("declared fixture cases"),
        );

        let full_report = manifest
            .get_mut("generation_reports")
            .and_then(toml::Value::as_table_mut)
            .and_then(|reports| reports.get_mut("full"))
            .and_then(toml::Value::as_table_mut)
            .expect("full report manifest");
        for (bucket, count) in [
            ("generated", 1),
            ("unsupported", 0),
            ("expected_fail", 0),
            ...
            full_report.insert(bucket.to_string(), toml::Value::Integer(count));
        }

        toml::to_string(&manifest).expect("synthetic manifest serialization")
    }
    ...
    fn centralized_provenance_report_fixture(
        ...
        let manifest_raw = test_manifest_with_declared_fixture();
        let manifest = parse_corpus_manifest(&manifest_raw).expect("test manifest");
    ...
    #[test]
    fn centralized_provenance_accepts_manifest_described_inventory() {
        let (root, config, manifest) = centralized_provenance_report_fixture(
            "manifest-described-inventory",
            "CURRENT",
            json!([]),
            "CURRENT",
        );
        let [declared] = manifest.cases.as_slice() else {
            panic!("synthetic manifest must declare one fixture");
        };
        assert_eq!(declared.id, "block/example");
        assert_eq!(declared.source, "block/example.html");
        assert_eq!(declared.status, CorpusStatus::Active);
        validate_generation_report_freshness(&config, &manifest)
            .expect("manifest-described report/XML inventory");
        fs::remove_dir_all(root).expect("test root cleanup");
    }
