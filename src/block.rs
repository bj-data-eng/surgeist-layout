...
use super::{
    ...
    LayoutErrorOf, LayoutErrorSiteOf, LayoutInputOf, LayoutInvalidInputOf, LayoutMissingContext,
...
where
    ...
    S: LayoutScalar,
{
    (ComputeBlockInnerPhaseL97::<Tree, S, M>::RUN)(tree, node, input, inherited)
}

type ComputeBlockInnerPhaseL97Run<Tree, S, M> =
    fn(
        &mut Tree,
        <Tree as Traverse>::Node,
        ComputeInputOf<S>,
        Option<&InheritedFloatExclusions<S, <Tree as Traverse>::Node>>,
    ) -> LayoutResultOf<<Tree as Traverse>::Node, ComputeOutputOf<S>, S, M>;

struct ComputeBlockInnerPhaseL97<Tree, S, M>(core::marker::PhantomData<(Tree, S, M)>);

impl<Tree, S, M> ComputeBlockInnerPhaseL97<Tree, S, M>
where
    Tree: Compute<M, Scalar = S>,
    S: LayoutScalar,
{
    const RUN: ComputeBlockInnerPhaseL97Run<Tree, S, M> = |tree: &mut Tree,
                                                           node: <Tree as Traverse>::Node,
                                                           input: ComputeInputOf<S>,
                                                           inherited: Option<
        &InheritedFloatExclusions<S, <Tree as Traverse>::Node>,
    >| {
...
where
    ...
    S: LayoutScalar,
{
    (LayoutInFlowChildrenPhaseL1125::<Tree, S, M>::RUN)(
        tree, node, children, constants, input, pass,
    )
}

type LayoutInFlowChildrenPhaseL1125Run<Tree, S, M> = fn(
    &mut Tree,
    <Tree as Traverse>::Node,
    &[<Tree as Traverse>::Node],
    &Constants<S>,
    ComputeInputOf<S>,
    InFlowPassContext<'_, S, <Tree as Traverse>::Node>,
) -> LayoutResultOf<
    <Tree as Traverse>::Node,
...
struct LayoutInFlowChildrenPhaseL1125<Tree, S, M>(core::marker::PhantomData<(Tree, S, M)>);

impl<Tree, S, M> LayoutInFlowChildrenPhaseL1125<Tree, S, M>
where
    Tree: Compute<M, Scalar = S>,
    S: LayoutScalar,
{
    const RUN: LayoutInFlowChildrenPhaseL1125Run<Tree, S, M> =
        |tree: &mut Tree,
         node: <Tree as Traverse>::Node,
         children: &[<Tree as Traverse>::Node],
         constants: &Constants<S>,
         ...
         pass: InFlowPassContext<'_, S, <Tree as Traverse>::Node>| {
            ...
            let contribution_seed =
                super::ScrollRectOf::try_new(content_box_origin, content_box_size)
                    .map_err(|error| layout_own_geometry_error(node, input.run_mode(), error))?;
            ...
            while index < children.len() {
                ...
                let child_style = match tree.layout_input(child) {
                    ...
                    LayoutInputOf::InlineText(_) => {
                        ...
                        content_size.inline =
                            content_size.inline.max(placement_content_size.inline);
                        ...
                        cursor_block =
                            cursor_block + placement.logical_block_extent(constants.flow_axes);
                        static_positions.extend(placement.static_positions);
                        resolved_terminal_float_block_end =
                            placement.resolved_float_terminal_block_end;
                ...
                let child_border = constants
                    ...
                    .transpose_with_node(tree, child)?;
                let parent_logical_unresolved_margin =
                    constants.flow_axes.logical_edges(unresolved_margin);
                ...
                let child_logical_available =
                    child_flow_axes.logical_size(constants.available_content);
                ...
                if inherits_float_exclusions {
                    ...
                    let child_logical_location = LogicalPointOf::new(
                        in_flow_child_inline_offset(
                            logical_child_size,
                            logical_child_margin,
                            constants,
                        ),
                    ...
                    let child_logical_location = if child_style.clear == Clear::None {
                        ...
                        LogicalPointOf::new(
                            ...
                            float_exclusions
                                .clearance_block(preview_cursor_block, child_style.clear),
                        )
                    };
                    let inherited = float_exclusions.for_ordinary_child(child_logical_location);
                    output = tree.compute_child_with_inherited_float_exclusions(
                        child,
                        child_input,
                        inherited,
                    )?;
                    ...
                    if inline_size_is_auto {
                        let parent_non_auto_margin = parent_logical_unresolved_margin
                            .map(resolved_length_auto_fallback_zero);
                        let band_child_inline = (placement.available_inline
                            - parent_non_auto_margin.inline_sum())
                        .max(S::ZERO);
                ...
                if !child_style.float.is_none() {
                    let margin_box_inline =
                        logical_child_size.inline + logical_child_margin.inline_sum();
                    ...
                    let containing_size = constants.containing_size(logical_node_inner_size);
                    let logical_location = constants.flow_axes.logical_point(
                        float_location,
                        output.size,
                        containing_size,
                    );
                    ...
                    let float_block_end = logical_location.block
                        + logical_child_size.block
                        + logical_child_margin.block_end;
                ...
                let bottom_margin_set = output
                    ...
                    .collapse_with_margin(
                        child_margin.at_physical_side(constants.flow_axes.block_end()),
                    );
                let child_margin_can_collapse_with_parent =
                    child_margin_can_collapse_with_parent(&child_style);
                ...
                let child_inline_content = (logical_child_margin.inline_sum()
                    + logical_child_size.inline)
...
where
    ...
    S: LayoutScalar,
{
    (LayoutInlineRunChildrenPhaseL2021::<Tree, S, M>::RUN)(
        tree,
        container,
        run,
        context,
...
type LayoutInlineRunChildrenPhaseL2021Run<Tree, S, M> = fn(
    &mut Tree,
    <Tree as Traverse>::Node,
    &[<Tree as Traverse>::Node],
    InlineRunContext<'_, S>,
    &FloatExclusions<S, <Tree as Traverse>::Node>,
    &mut ScrollContributionAccumulatorOf<S>,
) -> LayoutResultOf<
    <Tree as Traverse>::Node,
...
struct LayoutInlineRunChildrenPhaseL2021<Tree, S, M>(core::marker::PhantomData<(Tree, S, M)>);

impl<Tree, S, M> LayoutInlineRunChildrenPhaseL2021<Tree, S, M>
where
    Tree: Compute<M, Scalar = S>,
    S: LayoutScalar,
{
    const RUN: LayoutInlineRunChildrenPhaseL2021Run<Tree, S, M> =
        |tree: &mut Tree,
         container: <Tree as Traverse>::Node,
         run: &[<Tree as Traverse>::Node],
         context: InlineRunContext<'_, S>,
         ...
         contributions: &mut ScrollContributionAccumulatorOf<S>| {
            ...
            for (offset, child) in run.iter().copied().enumerate() {
                ...
                let child_style = match tree.layout_input(child) {
                    ...
                    LayoutInputOf::InlineBoundary(_) => {
                        ...
                        participants.push(MixedInlineParticipantOf::Boundary(
                            inline_boundary_control(
                                source_index,
                                boundary,
                                available_inline_extent,
                            ),
            ...
            let report = layout_mixed_inline_run_with_band_source(
                ...
                |block_start, block_end| {
                    ...
                    let band = if !set_layout || provider_error.is_some() {
                        float_exclusions
                            .query_rectangular_line_band(query_block_start, query_block_end)
                    } else {
                ...
                |block, clear| {
                    float_exclusions.clearance_for_line_intent(cursor_block + block, clear)
                        - cursor_block
                },
            );
            ...
            let resolved_float_terminal_block_end = resolved_inline_float_terminal_block_end(
                &report,
                cursor_block,
                owned_float_block_end,
            );
...
where
    ...
    S: LayoutScalar,
{
    (LayoutAbsoluteChildrenPhaseL3125::<Tree, S, M>::RUN)(
        tree,
        container_node,
        children,
        static_positions,
...
type LayoutAbsoluteChildrenPhaseL3125Run<Tree, S, M> =
    fn(
        &mut Tree,
        <Tree as Traverse>::Node,
        &[<Tree as Traverse>::Node],
        &[(<Tree as Traverse>::Node, Point<S>)],
        Size<S>,
        &Constants<S>,
        &mut ScrollContributionAccumulatorOf<S>,
    ) -> LayoutResultOf<<Tree as Traverse>::Node, (), S, M>;

struct LayoutAbsoluteChildrenPhaseL3125<Tree, S, M>(core::marker::PhantomData<(Tree, S, M)>);

impl<Tree, S, M> LayoutAbsoluteChildrenPhaseL3125<Tree, S, M>
where
    Tree: Compute<M, Scalar = S>,
    S: LayoutScalar,
{
    const RUN: LayoutAbsoluteChildrenPhaseL3125Run<Tree, S, M> =
        |tree: &mut Tree,
         container_node: <Tree as Traverse>::Node,
         children: &[<Tree as Traverse>::Node],
         static_positions: &[(<Tree as Traverse>::Node, Point<S>)],
         ...
         contributions: &mut ScrollContributionAccumulatorOf<S>| {
            ...
            let max_area_start_x = (container.width - constants.effective_border.right)
                .max(constants.effective_border.left);
            let area_start_y = constants.effective_border.top + constants.scrollbar_gutter.top;
            let max_area_start_y = (container.height - constants.effective_border.bottom)
                .max(constants.effective_border.top);
            ...
            for (source_index, child) in children.iter().copied().enumerate() {
                ...
                let final_size = known_size
                    ...
                    .clamp_optional(min_size, max_size);
                let margin = resolve_absolute_margin(
                    unresolved_margin,
                    inset,
                    style_size,
                    final_size,
                ...
                let static_y_direction =
                    static_axis_direction(constants.flow_axes, PhysicalAxis::Vertical);
                let location = Point::new(
                    AbsoluteAxis {
                        start: inset.left,
                        end: inset.right,
...
type BlockConstantsNewPhaseRun<S, Tree, M> =
    fn(
        &Tree,
        <Tree as Traverse>::Node,
        &NodeInputOf<S>,
        ComputeInputOf<S>,
    ) -> LayoutResultOf<<Tree as Traverse>::Node, Constants<S>, S, M>;

struct BlockConstantsNewPhase<S, Tree, M>(core::marker::PhantomData<(S, Tree, M)>);

impl<S, Tree, M> BlockConstantsNewPhase<S, Tree, M>
where
    S: LayoutScalar,
    Tree: Compute<M, Scalar = S>,
{
    const RUN: BlockConstantsNewPhaseRun<S, Tree, M> =
        |tree: &Tree,
         node: <Tree as Traverse>::Node,
         style: &NodeInputOf<S>,
         input: ComputeInputOf<S>| {
            ...
            let blocks_margin_collapse = !style.item_is_replaced
                && style.overflow.establishes_independent_formatting_context();
            ...
            let available_content = input.available().zip_map(
                content_box_inset_size,
                |available, inset| match available {
...
impl<S: LayoutScalar> Constants<S> {
    ...
    where
        Tree: Compute<M, Scalar = S>,
    {
        (BlockConstantsNewPhase::<S, Tree, M>::RUN)(tree, node, style, input)
    }
}
