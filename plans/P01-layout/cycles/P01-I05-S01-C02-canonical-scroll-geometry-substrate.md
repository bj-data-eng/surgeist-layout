# P01-I05-S01-C02 Canonical Scroll Geometry Substrate
Status: complete
Cycle ID: `P01/I05/S01/C02`
...
commit `f479b5e5d23294eafb82f8ae7ee6c740ead752d0`, entry `P01/I05/S01/C02`.
## 1 Outcome
...
MSRV, root, siblings, unsafe code, or generator architecture. `T02` is the
...
## 4 Tasks
### 4.1 `P01/I05/S01/C02/T01` Finite Rectangles And Read-Only Geometry Carriers
...
T01 acceptance ends with focused and default-feature verification; the
generator-feature aggregate is deferred to T02 because the recurring T01 gate
failure is T02's confirmed trigger.
**Commands:**
```sh
...
### 4.2 `P01/I05/S01/C02/T02` Deterministic Generator Gate And Owner-Lease Release
...
```
**Dependency:** The confirmed recurring parallel-only T01 generator gate failure.
**Intended commit:** `fix(generator): release generation locks explicitly`.
### 4.3 `P01/I05/S01/C02/T03` Canonical Boxes Clips And Saturated Gutters
...
CARGO_NET_OFFLINE=true just verify-generator
```
**Dependency:** `T01` supplies validated rectangles and clip carriers; `T02` restores the configured gate.
**Intended commit:** `feat(layout): derive canonical scroll boxes`.
### 4.4 `P01/I05/S01/C02/T04` Shared Contribution And Origin-Aware Range Substrate
...
CARGO_NET_OFFLINE=true just verify-generator
```
**Dependency:** `T03` supplies the canonical scrollport and used-axis clips.
**Intended commit:** `feat(layout): add scroll contribution and range substrate`.
### 4.5 `P01/I05/S01/C02/T05` Single Canonical Factory And Source-Based Rounding
**Files:** `src/scroll.rs` and focused composition/rounding tests.
**Outcome:** Compose T01-T04 behind one private source record and one private
...
CARGO_NET_OFFLINE=true just verify-generator
```
**Dependency:** `T03` and `T04` complete every source derivation.
...
source/tests, T02's generator file, `NOTICE.md`, and `LICENSE-TAFFY.md`; no
