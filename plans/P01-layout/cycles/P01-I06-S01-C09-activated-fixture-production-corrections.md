# P01-I06-S01-C09 Activated-Fixture Production Corrections

Status: complete

Cycle ID: `P01/I06/S01/C09`
...
commit `49ede2ba2672a91f99ba193651dbb1350ede7b80`: `FRI-06.4 D-03`, `D-06`,
`D-07`, `D-10`, `D-12`, `D-13`, and `D-15`; intrinsic, bidi, control, flow, and
clear portions of `FRI-06.7` through `FRI-06.9`; `FRI-06.10` rows
`src/inline.rs`, `src/block.rs`, and Focused Rust tests; and `FRI-06.14`
acceptance items 4 through 9 and 16 through 18.
...
commit `0a666f8f698703cd7979194a7f75f834e4c9b522`, entry
`P01/I06/S01/C09` and the production-correction matrix in Activation Recovery
Evidence.

## 1 Outcome

Correct the production behavior reached by exactly 52 validated variants across
21 named sources. Preserve shaped-participant, intrinsic, bidi, writing-mode,
RTL, control, and logical-clear semantics through the public compute front door
without changing any fixture input or derived artifact.

## 2 Boundary

P01/I06/S01/C08 is published and remotely verified at the cycle base. Its finite
fixture adapter can lower and compare the reviewed facts. The invalid diagnostic
derivation was discarded and is not artifact lineage. Exact-base typed probes
and one bounded scoped diagnostic subsequently corrected the production matrix
without producing artifact lineage.
...
production-visible test surface. The three tables must be disjoint and their
union must equal the 52-row matrix above.
...
The reviewed sequence's fixed baseline-helper subset and
`fri06_forced_break_strut` are not production corrections: exact-base public-
compute probes satisfy their fixed geometry oracles. All 16 baseline rows stop
before Rust lowering in `unsupportedChildNodesReason` through
`isSignificantInlineWhitespace`; the subset has digest
`f9ac335e450b4ffd014ae91ef211e699b513676711f70e2c27414fb64f7455a3`.
C08 owns that helper correction and forced-break fixture activation. The 408
failures from the broad diagnostic are not a recovery matrix; aggregate parity
remains FRI-13-owned. No task may use those sets to add a case dynamically.
...
### 4.1 `P01/I06/S01/C09/T01` Correct Intrinsic And Packing Heights
...
### 4.2 `P01/I06/S01/C09/T02` Correct RTL Subgrid Alignment Projection
...
**Dependency:** T01 task-clean; intrinsic track geometry is stable before the
complete alignment cross product is projected.

**Intended commit:** `fix(layout): correct RTL subgrid alignment projection`.

### 4.3 `P01/I06/S01/C09/T03` Correct Vertical Control And Logical Clear Projection
...
**Dependency:** T02 task-clean; grid alignment projection is stable before the
...
The three explicit row tables must be disjoint, contain exactly
`12 + 32 + 8 = 52` rows across the exact 21 sources, and admit no dynamic
