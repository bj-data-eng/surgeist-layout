# P01-I05-S01-C06 Bounded Fixtures Comparator And Final Regeneration
Status: complete
Cycle ID: `P01/I05/S01/C06`
...
### 2.3 Current Evidence And Decisions
...
- At cycle entry, checked-in XML contained 96 explicit legacy cross-group pairs.
  T04's settled inputs and derived corpus remove that transition without extending
...
- No scoped ExistingPinned run occurred. T04 executed one unfiltered full run,
...
## 4 Tasks
### 4.1 `P01/I05/S01/C06/T01` Activate Range-Span Comparator Diagnostics
...
### 4.2 `P01/I05/S01/C06/T02` Serialize And Parse The Finite Scroll Fixture Contract
...
bounded transition until T04. The unchanged helper keeps old-corpus provenance
valid.
...
**Dependency:** No semantic dependency on T01; this task is independently
startable from the C05 base. It executes after T01 only because both tasks edit
...
### 4.3 `P01/I05/S01/C06/T03` Settle The Eleven Browser Sources
...
count assertions remain at the base values until T04. No pre-existing source,
...
Corpus validation is intentionally deferred: before T04 adds the matching
manifest records, the corpus checker must reject these new Surgeist sources as
unmanifested. T04's completed derivation ran `just corpus-check` only after the
records and full corpus were frozen.

**Dependency:** T02 settles every serializer and parser field used by the
sources.

**Intended commit:** `test(parity): add FRI-05 browser sources`.

### 4.4 `P01/I05/S01/C06/T04` Freeze Inputs And Derive The Final Corpus Once
...
**Dependency:** T03 and the block correction are task-clean; the target source must settle before the final replacement.
