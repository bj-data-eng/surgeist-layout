# P01-I02-S01-C02 Signed Scroll Coordinate Contract
Status: complete
Cycle ID: `P01/I02/S01/C02`
...
## 1 Boundary
Published `P01/I02/S01/C01` supplies the reviewed `FlowAxes` mapping at the cycle
...
### 3.1 `P01/I02/S01/C02/T01` - Validated Signed Coordinate Values
...
### 3.2 `P01/I02/S01/C02/T02` - Canonical FlowAxes Scroll Projection
...
Depends on: T01. Intended commit: `scroll: project signed coordinates through flow`.
### 3.3 `P01/I02/S01/C02/T03` - Geometry Migration And Public Closure
...
Depends on: T02. Intended commit: `scroll: migrate geometry to signed flow ranges`.
## 4 Completion
...
bash -lc 'set -euo pipefail; hits=$(rg -n -o "physical_scroll_range_from_overflow_rects|FlowRelativeScrollRangeOf[^[:space:]]*::try_new|\\.physical_scroll_range\\(" src/scroll.rs); test "$(wc -l <<<"$hits" | tr -d "[:space:]")" -eq 5; printf "%s\\n" "$hits"'
bash -lc 'if rg -n --pcre2 "pub (?:const )?fn (?:writing_mode|direction|range)\\b" src/scroll.rs || sed -n "/^pub struct ScrollGeometryOf/,/^}/p" src/scroll.rs | rg -n "^\\s+(?:writing_mode: WritingMode|direction: Direction|range: ScrollRangeOf)"; then exit 1; else rc=$?; test "$rc" -eq 1; fi'
