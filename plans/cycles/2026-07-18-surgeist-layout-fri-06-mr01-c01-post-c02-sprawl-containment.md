...
`plans/specs/2026-07-18-surgeist-layout-fri-06-mr01-post-c02-sprawl-containment.md`
...
specialized fakes, macros, FRI-06-C03 participants, shaped-text optimization,
...
### `MR01-C01-T1` Preserve Ordered Non-Box Role Classification Through One Helper
...
T2 so the validation extraction has its own exact range and review.

**Intended commit:** `refactor(layout): centralize non-box role validation`.

### `MR01-C01-T2` Preserve Oracle Behavior Through One Scalar-Generic Compute Impl
...
post-T1 task base, record the test-only diff digest, and run the focused command.
...
**Dependency:** T1 implementation and independent task review are clean.
...
The handoff records the published candidate as FRI-06-C03's base and preserves
