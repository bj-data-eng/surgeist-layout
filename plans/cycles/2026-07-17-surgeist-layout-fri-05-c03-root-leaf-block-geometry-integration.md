...
commit `45e12b360ececa1ea4518c4da7400f58331b3a3a`, entry `FRI-05-C03`.
...
against the one canonical factory; it is removed by C03-T5 and never becomes a
...
### `C03-T1` Canonical Production Output And Public Geometry Surface
...
root/block source adapter until T5.
...
**Dependency:** C03-T1 supplies the canonical carrier and factory front door.

**Intended commit:** `feat(layout): emit stable measured-leaf scroll geometry`.

### `C03-T3` Saturated Block Reservation And Monotone Auto Passes
...
**Dependency:** C03-T2 supplies the monotone state and cache identity.

**Intended commit:** `feat(layout): settle block scrollbar reservations`.

### `C03-T4` Shared Block Contribution And Retained Child Geometry
...
**Dependency:** C03-T3 supplies final effective boxes and stable pass ownership.

**Intended commit:** `fix(layout): unify block scroll contribution geometry`.

### `C03-T5` Root Rounding Cache And Legacy-Path Closure
...
**Dependency:** C03-T4 completes migrated block source facts and child geometry.
