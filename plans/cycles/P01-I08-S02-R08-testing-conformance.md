...
Prefix inventories are anchored against the full library list. In particular,
`grid::tests::` means `^grid::tests::`; Cargo's substring filter also selects
three `grid::subgrid::tests::` leaves and is not the inventory counter.
