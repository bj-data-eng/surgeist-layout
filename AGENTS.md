...
Use this guide for automated work in the `surgeist-layout` crate repo.
...
This repo owns the `surgeist-layout` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-layout` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-layout` owns layout algorithms, layout contracts, oracle/parity tests, and fixture tooling.
...
1. Confirm the work belongs in `surgeist-layout`.
