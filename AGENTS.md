# surgeist-layout Repository Guide
...
`surgeist-layout` is an independent leaf repository. It owns its manifest,
layout domain implementation, public front door, focused tests and docs, commits,
...
| Root integration MSRV, repository URL, and compatible pin | Root `Cargo.toml`, root `.gitmodules`, and root's committed gitlink for `crates/surgeist-layout`, when root integration is in scope |
...
`surgeist-layout` owns layout algorithms and contracts, layout-ready fixtures,
and parity and oracle tests. It excludes authored CSS and style resolution,
retained identity, text shaping, and rendering.
...
```sh
cargo check -p surgeist-layout
cargo test -p surgeist-layout
cargo clippy -p surgeist-layout --all-targets -- -F unsafe-code -D warnings
cargo fmt --check
```

For feature-gated generator and browser-parity commands, read `Cargo.toml` and
`tests/layout/browser_parity/README.md`; this inventory does not copy that
procedure.
